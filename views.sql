...
SELECT
  e.id as evento, s.prezzo_siae + s.cachet + l.prezzo + SUM(efs.prezzo) AS "spesa"
FROM
  spettacolo AS s,
  evento AS e,
...
WHERE
  ...
  AND efs.evento = e.id
  AND e.luogo = l.id
GROUP BY e.id, s.prezzo_siae,s.cachet,l.prezzo;
...
FROM
  spesa_evento AS se
NATURAL JOIN incasso_evento as ie
GROUP BY ie.evento, guadagno;
