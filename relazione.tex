...
\begin{tabularx}{\textwidth}{|X|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
\hline
  \textbf{Entit\`a - Relazione} & \textbf{Traduzione}\\
\hline
  Artista & Artista(\underline{id},name) \\
\hline
  Spettacolo & Spettacolo(\underline{id},title,artist,siae_price,cachet) \\
\hline
  Luogo & Luogo(\underline{id},venue_type,name,coords,price) \\
\hline
  Evento & Evento(\underline{id},show,venue,title,starts_at,ends_at) \\
\hline
  Settore & Settore(\underline{id},venue,name,capacity) \\
\hline
  Posto & Posto(\underline{id},sector,row,col) \\
\hline
  Fornitore & Fornitore(\underline{name},description) \\
\hline
  Biglietto & Biglietto(\underline{id},seat,event) \\
\hline
  Costo & Costo(\underline{sector},\underline{event},price) \\
\hline
  Servizio & Servizio(\underline{provider},\underline{type},\underline{event},price) \\
\hline
\end{tabularx}
