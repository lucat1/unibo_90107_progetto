...
Un artista può essere una persona o un gruppo a cui appartengo più persone.\\
...
<<<<<<< HEAD
\begin{itemize}
  \item (RV1) Il cachet e il prezzo SIAE devono essere maggiori o uguali a 0.
  \item (RV2) Il prezzo di affitto del luogo deve essere maggiore o uguale a 0.
  \item (RV3) La capacità di un settore deve essere maggiore di 0.
  \item (RV4) Il prezzo di un settore deve essere maggiore o uguale a 0.
  \item (RV5) Il prezzo di un servizio deve essere maggiore di 0.
...
\begin{itemize}
  ...
  \item (RD2) Il costo di un evento è la somma tra il cachet e il costo SIAE, il costo di affitto del luogo e i costi dei servizi.
...
\begin{enumerate}[label=$\bullet$\hspace{.5em}(RV\arabic*),leftmargin=6em,ref=RV\theenumi]
  ...
  \item \label{rv8} Tutti i settori collegati allo stesso evento devono appartenere
    allo stesso luogo.
\end{enumerate}
...
\begin{tabularx}{\textwidth}{|X|X|>{\raggedright\arraybackslash}X|}
\hline
...
\hline
Costo & R & 600 000 (= eventi · settori)\\
\hline
Vendita & R & 30 000 000 ($\leq$ eventi · posti)\\
\hline
Prenotazione & R & 30 000 000 (= vendita)\\
\hline
\end{tabularx}
...
Dal diagramma ER traspare una evidente ridondanza nella relazione \emph{Posizione},
che lega un Evento ad un Luogo. Per trovare il luogo in cui si tiene un dato evento
si pu\`o anche sfruttare la relazione \emph{Costo} e la relazione \emph{Divione}.
Poich\`e quando si visualizza un evento si vuole molto spesso conoscere anche
il luogo in cui si svolge, l'utilizzo di questa operazione \`e molto alto.
Per migliorare le prestazioni abbiamo preferito dunque lasciare una relazione
ridondante, imponendo gli opportuni vincoli affinch\`e non si verifichino
inconsistenze (vedi \ref{rv7}, \ref{rv8}).
